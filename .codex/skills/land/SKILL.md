...
2. Determine the base branch from the PR itself (`gh pr view --json baseRefName`)
   and treat that as the source of truth.
3. Confirm the full gauntlet is green locally before any push.
4. If the working tree has uncommitted changes, commit with the `commit` skill
   and push with the `push` skill before proceeding.
5. Check mergeability and conflicts against the base branch.
...
if [ "$mergeable" = "CONFLICTING" ]; then
  # Run the `pull` skill to handle fetch + merge of origin/$base_branch and
  # conflict resolution.
